...
class MaterialDemand(SafeDeleteModel):
    ...
    demand_no = models.CharField(max_length=50, blank=True, null=True, unique=True)
    material = models.ForeignKey("core.Material", related_name='demands' ,on_delete=models.PROTECT, null=False, blank=False)
...
class ProcurementOrder(SafeDeleteModel):
    ...
    vendor = models.ForeignKey("core.Company", on_delete=models.CASCADE, blank=False, null=True)
    payment_term = models.CharField(max_length=20, choices=PAYMENT_TERMS, verbose_name="Vade Şartı")
    payment_method = models.CharField(max_length=20, choices=PAYMENT_METHODS, verbose_name="Ödeme Yöntemi")
    incoterms = models.CharField(max_length=20, choices=INCOTERMS, verbose_name="Teslim Şekli (Incoterms)")
    ...
    due_discount = models.DecimalField(
        max_digits=4,
        decimal_places=3,
        default=Decimal("0.000"),
        validators=[
    ...
    delivery_address = models.CharField(max_length=250, null=True, blank=False)
    history = HistoricalRecords()
    
    @property
    def total_price_without_tax(self):
        # Sum of unit_price * quantity for all lines, minus trade_discount and due_discount
        subtotal = sum((line.unit_price * (line.quantity) for line in self.lines.all()), Decimal("0.00"))
    ...
    def total_price_with_tax(self):
        ...
        tax_total = sum((line.unit_price * (line.quantity) * (line.tax_rate) for line in self.lines.all()), Decimal("0.00"))
...
class ProcurementInvoice(SafeDeleteModel):
    ...
    po = models.ForeignKey("procurement.ProcurementOrder", related_name='invoices', on_delete=models.CASCADE)
    
class ProcurementOrderLine(SafeDeleteModel):
    _safedelete_policy = SOFT_DELETE
    po = models.ForeignKey("procurement.ProcurementOrder", related_name='lines', on_delete=models.CASCADE)
    material = models.ForeignKey("core.Material", on_delete=models.CASCADE, null=False, blank=False)
    uom = UOMField(null=False, blank=False)
    quantity = models.DecimalField(max_digits=21, 
                                   decimal_places=2,
                                   null=False,
                                   blank=False,
                                   validators=[MinValueValidator(0.00)])
    quantity_received = models.DecimalField(max_digits=21, 
                                   decimal_places=2,
                                   null=False,
                                   blank=False,
                                   validators=[MinValueValidator(0.00)])
    unit_price = models.DecimalField(max_digits=32, decimal_places=2)
