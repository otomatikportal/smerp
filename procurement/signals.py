            ...
            if not VariableCost.objects.filter(procurement_order=instance, material=line.material).exists():
                ...
                if original_currency != 'TRY':
                    ...
                    if try_cost is not None:
                        final_cost = try_cost
                        final_currency = 'TRY'
                        rate_used = CurrencyExchangeRate.get_rate(original_currency, 'TRY', today)
                        logger.info(f"Converted cost: {original_cost} {original_currency} → {final_cost} TRY using rate {rate_used} for {today}")
                    else:
                        logger.warning(f"No exchange rate found for {original_currency} to TRY on {today}. Searching for latest available rate...")
                        ...
                        if try_cost_latest is not None:
                            ...
                            if latest_rate_obj:
                                ...
                                logger.warning(f"AUDIT: Used latest available rate from {rate_date} (rate: {rate_value}) to convert {original_cost} {original_currency} → {final_cost} TRY. Current date: {today}")
                            else:
                                logger.warning(f"AUDIT: Converted using latest rate {original_cost} {original_currency} → {final_cost} TRY, but could not determine rate date")
                        else:
                            final_cost = original_cost
                            final_currency = original_currency
                            logger.error(f"AUDIT: NO EXCHANGE RATES AVAILABLE for {original_currency} to TRY. Using original cost: {original_cost} {original_currency}. This may cause inconsistent costing!")
                else:
                    final_cost = original_cost
                    final_currency = 'TRY'
